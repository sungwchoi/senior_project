# import os, sys, argparse, json   # ### CHANGED (json)
# import numpy as np
# import torch, cv2
# import rclpy
# from rclpy.node import Node
# from rclpy.qos import QoSProfile, QoSReliabilityPolicy, QoSHistoryPolicy
# from sensor_msgs.msg import Image
# from vision_msgs.msg import Detection2DArray, Detection2D, ObjectHypothesisWithPose, BoundingBox2D
# from cv_bridge import CvBridge
# from rclpy.utilities import remove_ros_args

# class YoloNode(Node):
#     def __init__(self):
#         super().__init__('yolo_node')

#         # -------- 파라미터 선언 --------
#         p = self.declare_parameter
#         self.weights = p('weights', '').get_parameter_value().string_value
#         self.repo_path = p('repo_path', '').get_parameter_value().string_value
#         self.device = p('device', 'cpu').get_parameter_value().string_value
#         self.img_size = int(p('img_size', 640).get_parameter_value().integer_value or 640)
#         self.conf_thres = float(p('conf_thres', 0.25).get_parameter_value().double_value or 0.25)
#         self.iou_thres = float(p('iou_thres', 0.45).get_parameter_value().double_value or 0.45)
#         self.max_det = int(p('max_det', 100).get_parameter_value().integer_value or 100)
#         self.stride = int(p('stride', 1).get_parameter_value().integer_value or 1)

#         # 토픽 파라미터 (키 호환)
#         source_topic = p('source_topic', '').get_parameter_value().string_value
#         subscribe_image = p('subscribe_image', '').get_parameter_value().string_value
#         self.source_topic = subscribe_image or source_topic or '/camera/image_raw'

#         annotated_topic = p('annotated_topic', '').get_parameter_value().string_value
#         publish_overlay = p('publish_overlay', '').get_parameter_value().string_value
#         self.annotated_topic = publish_overlay or annotated_topic or '/robot/yolo/annotated'

#         detections_topic = p('detections_topic', '').get_parameter_value().string_value
#         publish_boxes = p('publish_boxes', '').get_parameter_value().string_value
#         self.dets_topic = publish_boxes or detections_topic or '/robot/yolo/detections'

#         self.publish_annotated = p('publish_annotated', True).get_parameter_value().bool_value

#         # --------- ### NEW: 그리기/팔레트 관련 파라미터 ----------
#         self.box_thickness  = int(p('box_thickness', 2).get_parameter_value().integer_value or 2)
#         self.font_scale     = float(p('font_scale', 0.5).get_parameter_value().double_value or 0.5)
#         self.font_thickness = int(p('font_thickness', 1).get_parameter_value().integer_value or 1)
#         self.use_hash_color = p('use_hash_color', True).get_parameter_value().bool_value
#         self.palette_json   = p('palette_json', '').get_parameter_value().string_value

#         # 기본 팔레트 (RGB로 정의, OpenCV에서는 BGR로 변환해서 사용)
#         self.palette_rgb = {
#             #'person':      (255, 255,   0),  # 노랑
#             'weapons':     (  0,   0, 255),  # 파랑
#             'chair':         (255,   0,   0),  # 빨강
#             'bottle':  (  0, 255, 255),  # 하늘
#             #'bus':         (  0, 165, 255),  # 주황
#             'person':       (255, 105, 180),  # 분홍
#         }
#         # palette_json 주면 덮어쓰기 (RGB 배열로 받음)
#         if self.palette_json:
#             try:
#                 user_map = json.loads(self.palette_json)
#                 for k, v in user_map.items():
#                     if isinstance(v, (list, tuple)) and len(v) == 3:
#                         r, g, b = int(v[0]), int(v[1]), int(v[2])
#                         r = max(0, min(255, r)); g = max(0, min(255, g)); b = max(0, min(255, b))
#                         self.palette_rgb[str(k)] = (r, g, b)
#             except Exception as e:
#                 self.get_logger().warn(f"palette_json parse failed: {e}")

#         # -------- YOLOv5 import 경로 --------
#         if self.repo_path and self.repo_path not in sys.path:
#             sys.path.insert(0, self.repo_path)
#         try:
#             from models.common import DetectMultiBackend
#             from utils.torch_utils import select_device
#             from utils.augmentations import letterbox
#             from utils.general import non_max_suppression, scale_boxes
#         except Exception as e:
#             self.get_logger().error(f"Failed to import yolov5 modules: {e}")
#             raise
#         self.DetectMultiBackend = DetectMultiBackend
#         self.select_device = select_device
#         self.letterbox = letterbox
#         self.nms = non_max_suppression
#         self.scale_boxes = scale_boxes

#         # -------- 모델 로드 --------
#         if not self.weights or not os.path.exists(self.weights):
#             self.get_logger().warn(f"weights not found: '{self.weights}'. Set param 'weights' correctly.")
#         self.bridge = CvBridge()
#         self.dev = self.select_device(self.device)
#         self.model = self.DetectMultiBackend(self.weights, device=self.dev, dnn=False, data=None, fp16=False)
#         self.names = getattr(self.model, 'names', [str(i) for i in range(1000)])

#         # -------- 통신 설정 --------
#         qos = QoSProfile(depth=2,
#                          reliability=QoSReliabilityPolicy.BEST_EFFORT,
#                          history=QoSHistoryPolicy.KEEP_LAST)
#         self.sub = self.create_subscription(Image, self.source_topic, self.on_image, qos)
#         self.pub_dets = self.create_publisher(Detection2DArray, self.dets_topic, 10)
#         self.pub_img  = self.create_publisher(Image, self.annotated_topic, 10) if self.publish_annotated else None

#         self.frame_idx = 0
#         self.get_logger().info(
#             f"YOLO ready | weights={self.weights} device={self.device} "
#             f"src={self.source_topic} dets={self.dets_topic} ann={self.annotated_topic}"
#         )

#     # ### NEW: 클래스명 → BGR 색상 얻기
#     def _color_for_class(self, class_name: str):
#         if class_name in self.palette_rgb:
#             r, g, b = self.palette_rgb[class_name]
#             return (b, g, r)  # OpenCV는 BGR
#         if self.use_hash_color:
#             h = abs(hash(class_name)) % (256*256*256)
#             r = (h >> 16) & 0xFF
#             g = (h >> 8 ) & 0xFF
#             b = (h      ) & 0xFF
#             # 너무 어두우면 밝게
#             if (r + g + b) < 140:
#                 r = min(255, r+80); g = min(255, g+80); b = min(255, b+80)
#             return (b, g, r)
#         # fallback: green
#         return (0, 255, 0)

#     def on_image(self, msg: Image):
#         self.frame_idx += 1
#         if self.stride > 1 and (self.frame_idx % self.stride) != 0:
#             return

#         try:
#             cv_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
#         except Exception as e:
#             self.get_logger().warning(f"cv_bridge failed: {e}")
#             return

#         im = self.letterbox(cv_img, self.img_size, stride=32, auto=True)[0]
#         im = im[:, :, ::-1].transpose(2, 0, 1)
#         im = np.ascontiguousarray(im)
#         im = torch.from_numpy(im).to(self.dev).float() / 255.0
#         if im.ndimension() == 3:
#             im = im.unsqueeze(0)

#         with torch.no_grad():
#             pred = self.model(im, augment=False, visualize=False)
#             det = self.nms(pred, self.conf_thres, self.iou_thres, classes=None, max_det=self.max_det)[0]

#         dets = Detection2DArray()
#         dets.header = msg.header
#         annotated = cv_img.copy()

#         if len(det):
#             det[:, :4] = self.scale_boxes(im.shape[2:], det[:, :4], cv_img.shape).round()
#             for *xyxy, conf, cls in det.cpu().numpy():
#                 x1, y1, x2, y2 = map(float, xyxy)
#                 w, h = x2 - x1, y2 - y1
#                 d = Detection2D()
#                 d.header = msg.header
#                 d.bbox = BoundingBox2D()
#                 d.bbox.center.position.x = x1 + w/2.0
#                 d.bbox.center.position.y = y1 + h/2.0
#                 d.bbox.size_x = w; d.bbox.size_y = h
#                 hyp = ObjectHypothesisWithPose()
#                 cname = self.names[int(cls)] if int(cls) < len(self.names) else str(int(cls))
#                 hyp.hypothesis.class_id = cname
#                 hyp.hypothesis.score = float(conf)
#                 d.results.append(hyp)
#                 dets.detections.append(d)

#                 if self.publish_annotated:
#                     color = self._color_for_class(cname)   # ### CHANGED
#                     cv2.rectangle(annotated, (int(x1), int(y1)), (int(x2), int(y2)), color, self.box_thickness)
#                     label = f"{cname} {conf:.2f}"
#                     (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, self.font_scale, self.font_thickness)
#                     tx1, ty1 = int(x1), max(0, int(y1) - th - 6)
#                     tx2, ty2 = int(x1) + tw + 8, int(y1)
#                     cv2.rectangle(annotated, (tx1, ty1), (tx2, ty2), color, -1)
#                     cv2.putText(annotated, label, (int(x1)+4, int(y1)-6),
#                                 cv2.FONT_HERSHEY_SIMPLEX, self.font_scale, (255,255,255), self.font_thickness, cv2.LINE_AA)

#         self.pub_dets.publish(dets)
#         if self.publish_annotated and self.pub_img and self.pub_img.get_subscription_count() > 0:
#             try:
#                 self.pub_img.publish(self.bridge.cv2_to_imgmsg(annotated, encoding='bgr8'))
#             except Exception as e:
#                 self.get_logger().warning(f"annotated publish failed: {e}")

# def main():
#     # ROS 인자 제거 → argparse가 종료하지 않도록
#     clean_argv = remove_ros_args(sys.argv)

#     parser = argparse.ArgumentParser(add_help=False, allow_abbrev=False)
#     # 알 수 없는 인자는 무시
#     _, _unknown = parser.parse_known_args(clean_argv[1:])

#     rclpy.init(args=None)
#     node = YoloNode()
#     try:
#         rclpy.spin(node)
#     except KeyboardInterrupt:
#         pass
#     finally:
#         node.destroy_node()
#         rclpy.shutdown()

# if __name__ == '__main__':
#     main()



#!/usr/bin/env python3
import os, sys, argparse, json
import numpy as np
import torch, cv2
import rclpy
from rclpy.node import Node
from rclpy.qos import QoSProfile, QoSReliabilityPolicy, QoSHistoryPolicy
from rclpy.utilities import remove_ros_args
from sensor_msgs.msg import Image
from vision_msgs.msg import Detection2DArray, Detection2D, ObjectHypothesisWithPose, BoundingBox2D
from std_msgs.msg import String
from cv_bridge import CvBridge

class YoloNode(Node):
    def __init__(self):
        super().__init__('yolo_node')
        p = self.declare_parameter

        # -------- 파라미터 --------
        self.weights    = p('weights', '').get_parameter_value().string_value
        self.repo_path  = p('repo_path', '').get_parameter_value().string_value
        self.device     = p('device', 'cpu').get_parameter_value().string_value
        self.img_size   = int(p('img_size', 640).get_parameter_value().integer_value or 640)
        self.conf_thres = float(p('conf_thres', 0.25).get_parameter_value().double_value or 0.25)
        self.iou_thres  = float(p('iou_thres', 0.45).get_parameter_value().double_value or 0.45)
        self.max_det    = int(p('max_det', 100).get_parameter_value().integer_value or 100)
        self.stride     = int(p('stride', 1).get_parameter_value().integer_value or 1)

        # 토픽 이름(런치 호환)
        subscribe_image = p('subscribe_image', '').get_parameter_value().string_value
        source_topic    = p('source_topic', '').get_parameter_value().string_value
        self.source_topic = subscribe_image or source_topic or '/camera/image_raw'

        publish_overlay   = p('publish_overlay', '').get_parameter_value().string_value
        annotated_topic   = p('annotated_topic', '').get_parameter_value().string_value
        self.annotated_topic = publish_overlay or annotated_topic or '/robot/yolo/annotated'

        publish_boxes   = p('publish_boxes', '').get_parameter_value().string_value
        detections_topic = p('detections_topic', '').get_parameter_value().string_value
        self.dets_topic = publish_boxes or detections_topic or '/robot/yolo/detections'

        self.publish_annotated = p('publish_annotated', True).get_parameter_value().bool_value

        # NEW: 감지 클래스 매핑(콤마 구분, 소문자 비교)
        person_labels_csv = p('yolo_person_labels', 'person').get_parameter_value().string_value
        weapon_labels_csv = p('yolo_weapon_labels', 'weapon,gun,knife').get_parameter_value().string_value
        self.person_labels = {t.strip().lower() for t in person_labels_csv.split(',') if t.strip()}
        self.weapon_labels = {t.strip().lower() for t in weapon_labels_csv.split(',') if t.strip()}

        # NEW: 그리기 옵션
        self.box_thickness  = int(p('box_thickness', 2).get_parameter_value().integer_value or 2)
        self.font_scale     = float(p('font_scale', 0.5).get_parameter_value().double_value or 0.5)
        self.font_thickness = int(p('font_thickness', 1).get_parameter_value().integer_value or 1)
        self.use_hash_color = p('use_hash_color', True).get_parameter_value().bool_value
        self.palette_json   = p('palette_json', '').get_parameter_value().string_value

        self.palette_rgb = {
            'person': (255, 215, 0),   # yellow
            'weapon': (0, 0, 255),     # red-ish (BGR 변환되니 의미만)
            'gun':    (0, 0, 255),
            'knife':  (0, 0, 255),
        }
        if self.palette_json:
            try:
                user_map = json.loads(self.palette_json)
                for k, v in user_map.items():
                    if isinstance(v, (list, tuple)) and len(v) == 3:
                        r, g, b = map(int, v)
                        self.palette_rgb[str(k)] = (max(0,min(255,r)), max(0,min(255,g)), max(0,min(255,b)))
            except Exception as e:
                self.get_logger().warn(f"palette_json parse failed: {e}")

        # -------- YOLOv5 import --------
        if self.repo_path and self.repo_path not in sys.path:
            sys.path.insert(0, self.repo_path)
        try:
            from models.common import DetectMultiBackend
            from utils.torch_utils import select_device
            from utils.augmentations import letterbox
            from utils.general import non_max_suppression, scale_boxes
        except Exception as e:
            self.get_logger().error(f"Failed to import yolov5 modules: {e}")
            raise
        self.DetectMultiBackend = DetectMultiBackend
        self.select_device = select_device
        self.letterbox = letterbox
        self.nms = non_max_suppression
        self.scale_boxes = scale_boxes

        # -------- 모델 로드 --------
        if not self.weights or not os.path.exists(self.weights):
            self.get_logger().warn(f"weights not found: '{self.weights}'")
        self.bridge = CvBridge()
        self.dev = self.select_device(self.device)
        self.model = self.DetectMultiBackend(self.weights, device=self.dev, dnn=False, data=None, fp16=False)
        self.names = getattr(self.model, 'names', [str(i) for i in range(1000)])

        # -------- 통신 --------
        qos = QoSProfile(depth=2, reliability=QoSReliabilityPolicy.BEST_EFFORT, history=QoSHistoryPolicy.KEEP_LAST)
        self.sub = self.create_subscription(Image, self.source_topic, self.on_image, qos)
        self.pub_dets = self.create_publisher(Detection2DArray, self.dets_topic, 10)
        self.pub_img  = self.create_publisher(Image, self.annotated_topic, 10) if self.publish_annotated else None
        self.pub_event = self.create_publisher(String, '/robot/yolo_event', 10)  # NEW

        self.frame_idx = 0
        self._last_event_frame = -1
        self.get_logger().info(
            f"YOLO ready | weights={self.weights} device={self.device} "
            f"src={self.source_topic} dets={self.dets_topic} ann={self.annotated_topic}"
        )

    def _color_for_class(self, class_name: str):
        key = class_name.lower()
        if key in self.palette_rgb:
            r, g, b = self.palette_rgb[key]
            return (b, g, r)  # OpenCV는 BGR
        if self.use_hash_color:
            h = abs(hash(key)) % (256*256*256)
            r = (h >> 16) & 0xFF; g = (h >> 8) & 0xFF; b = h & 0xFF
            if (r + g + b) < 140:
                r = min(255, r+80); g = min(255, g+80); b = min(255, b+80)
            return (b, g, r)
        return (0, 255, 0)

    def on_image(self, msg: Image):
        self.frame_idx += 1
        if self.stride > 1 and (self.frame_idx % self.stride) != 0:
            return

        try:
            cv_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        except Exception as e:
            self.get_logger().warning(f"cv_bridge failed: {e}")
            return

        im = self.letterbox(cv_img, self.img_size, stride=32, auto=True)[0]
        im = im[:, :, ::-1].transpose(2, 0, 1)
        im = np.ascontiguousarray(im)
        im = torch.from_numpy(im).to(self.dev).float() / 255.0
        if im.ndimension() == 3:
            im = im.unsqueeze(0)

        with torch.no_grad():
            pred = self.model(im, augment=False, visualize=False)
            det = self.nms(pred, self.conf_thres, self.iou_thres, classes=None, max_det=self.max_det)[0]

        dets = Detection2DArray()
        dets.header = msg.header
        annotated = cv_img.copy()

        saw_person = False
        saw_weapon = False

        if len(det):
            det[:, :4] = self.scale_boxes(im.shape[2:], det[:, :4], cv_img.shape).round()
            for *xyxy, conf, cls in det.cpu().numpy():
                x1, y1, x2, y2 = map(float, xyxy)
                w, h = x2 - x1, y2 - y1

                cname = self.names[int(cls)] if int(cls) < len(self.names) else str(int(cls))
                cname_l = cname.lower()
                if cname_l in self.person_labels: saw_person = True
                if cname_l in self.weapon_labels: saw_weapon = True

                d = Detection2D()
                d.header = msg.header
                d.bbox = BoundingBox2D()
                d.bbox.center.position.x = x1 + w/2.0
                d.bbox.center.position.y = y1 + h/2.0
                d.bbox.size_x = w; d.bbox.size_y = h
                hyp = ObjectHypothesisWithPose()
                hyp.hypothesis.class_id = cname
                hyp.hypothesis.score = float(conf)
                d.results.append(hyp)
                dets.detections.append(d)

                if self.publish_annotated:
                    color = self._color_for_class(cname)
                    cv2.rectangle(annotated, (int(x1), int(y1)), (int(x2), int(y2)), color, self.box_thickness)
                    label = f"{cname} {conf:.2f}"
                    (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, self.font_scale, self.font_thickness)
                    tx1, ty1 = int(x1), max(0, int(y1) - th - 6)
                    tx2, ty2 = int(x1) + tw + 8, int(y1)
                    cv2.rectangle(annotated, (tx1, ty1), (tx2, ty2), color, -1)
                    cv2.putText(annotated, label, (int(x1)+4, int(y1)-6),
                                cv2.FONT_HERSHEY_SIMPLEX, self.font_scale, (255,255,255), self.font_thickness, cv2.LINE_AA)

        # 이벤트 발행(프레임당 최대 1회)
        if self._last_event_frame != self.frame_idx:
            send = None
            if saw_weapon: send = 'WEAPON'
            elif saw_person: send = 'PERSON'
            if send:
                self.pub_event.publish(String(data=send))
                self.get_logger().info(f"[YOLO-EVENT] {send}")
                self._last_event_frame = self.frame_idx

        self.pub_dets.publish(dets)
        if self.publish_annotated and self.pub_img and self.pub_img.get_subscription_count() > 0:
            try:
                self.pub_img.publish(self.bridge.cv2_to_imgmsg(annotated, encoding='bgr8'))
            except Exception as e:
                self.get_logger().warning(f"annotated publish failed: {e}")

def main():
    clean_argv = remove_ros_args(sys.argv)
    parser = argparse.ArgumentParser(add_help=False, allow_abbrev=False)
    _, _ = parser.parse_known_args(clean_argv[1:])
    rclpy.init(args=None)
    node = YoloNode()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()
        rclpy.shutdown()

if __name__ == '__main__':
    main()

